...
/**
 * Class MovementDirectionSetsController
 * @property mixed MovementDirectionSet
 */
class MovementDirectionSetsController extends AppController {
	...
	public function add() {
        ...
        if ($this->request->is('post')) {
			...
			//Create the user
            $this->MovementDirectionSet->create();
			
			//See if we can save the user using the given data...
			if ($this->MovementDirectionSet->save($this->request->data)) {
				//If the user has been saved, indicate as much and do a
				//redirect.
				$this->Session->setFlash(__('Movement Direction Set Saved'));
