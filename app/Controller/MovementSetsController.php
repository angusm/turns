...
/**
 * Class MovementSetsController
 * @property MovementSet $MovementSet
 */
class MovementSetsController extends AppController {
	...
	public function add() {
        ...
        if ($this->request->is('post')) {
			...
			//Create the user
            $this->MovementSet->create();
			
			//See if we can save the user using the given data...
			if ($this->MovementSet->save($this->request->data)) {
				//If the user has been saved, indicate as much and do a
				//redirect.
				$this->Session->setFlash(__('Movement Set Saved'));
